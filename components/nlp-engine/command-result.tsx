...
import { DesktopCommandResult } from "./desktop-command-result"
...
export function CommandResult({ intent, response, commandId }: CommandResultProps) {
  ...
  // Check if this is a desktop app command with results
  const isDesktopCommand =
    status === "completed" &&
    result &&
    result.success !== undefined &&
    intent.target &&
    ["email", "browser", "crm", "files"].includes(intent.target.toLowerCase())
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
      <Card>
        ...
        <CardContent>
          ...
          {/* Show desktop command result if available */}
          {isDesktopCommand && (
            <div className="mt-4">
              <DesktopCommandResult
                app={intent.target || "unknown"}
          ...
          {/* Show regular result if available and not a desktop command */}
          {status === "completed" && result && !isDesktopCommand && (
          ...
          {status === "failed" && result && !isDesktopCommand && (
