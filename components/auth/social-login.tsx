  ...
  useEffect(() => {
    const initFirebase = async () => {
      try {
        ...
        console.error("Error initializing Firebase in SocialLogin:", error)
        toast({
  ...
  const handleGoogleLogin = async () => {
    if (!isInitialized) {
      toast({
        ...
        description: "Please wait while we initialize Firebase or refresh the page.",
        variant: "destructive",
      })
      return
    }
    ...
    <div className="grid grid-cols-3 gap-3 mt-4">
      ...
      <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
        <Button
          ...
          disabled={isGithubLoading || !isInitialized}
        >
