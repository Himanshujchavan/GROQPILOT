...
export const initializeFirebase = () => {
  if (typeof window === "undefined") return { app: null, auth: null, db: null } // Skip on server-side
  ...
  try {
    ...
    if (!getApps().length) {
      ...
      console.log("Firebase already initialized, getting app instance")
      app = getApp()
    }
...
// Add functions for storing and updating commands
export const storeCommand = async (intent: any, response: string) => {
  try {
    ...
    return { id: commandRef.id, ...(await commandRef.get()).data() }
  } catch (error) {
...
export const updateCommandStatus = async (commandId: string, status: any, data?: any) => {
  try {
    if (!db || !auth) return

    const commandRef = doc(db, "commands", commandId)
    await updateDoc(commandRef, { status, result: data, updatedAt: serverTimestamp() })
  } catch (error) {
